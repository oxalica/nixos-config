{
  ...
  inputs = {
    ...
    nixpkgs-logseq2.url = "github:NixOS/nixpkgs/pull/516682/head";
        ...
        system-label = {
          ...
          system.nixos.label =
            if self.sourceInfo ? lastModifiedDate && self.sourceInfo ? shortRev then
              "${lib.substring 0 8 self.sourceInfo.lastModifiedDate}.${self.sourceInfo.shortRev}"
            else
              lib.warn "Repo is dirty, revision will not be available in system label" "dirty";
        };
    ...
    in
    {
      ...
      nixosConfigurations = {
        ...
        lithium = mkSystem "lithium" "x86_64-linux" inputs.nixpkgs {
          extraModules = with nixosModules; [
            sops
            inputs.blahrs.nixosModules.blahd
          ];
        };
    ...
    // flake-utils.lib.eachDefaultSystem (system: rec {
      ...
      devShells.default =
        ...
        mkShellNoCC {
          packages = [
            nvfetcher
            packages.nixos-rebuild-shortcut
          ];
        };
